...
fn is_report_safe(report: Report) -> bool {
    ...
    let all_decreasing = pairwise.clone().all(|(c, n)| {
        let diff: i32 = (c - n).abs();
        (c >= *n) && (1 <= diff) && (diff <= 3)
    });
    let all_increasing = pairwise.clone().all(|(c, n)| {
        let diff: i32 = (c - n).abs();
        (c <= *n) && (1 <= diff) && (diff <= 3)
    });
...
fn problem_dampener(report: Report) -> bool {
    report.iter().cloned().enumerate().any(|(index, _value)| {
...
pub fn main() {
    ...
    println!(
        ...
        reports
            .iter()
            .map(|r: &Report| { is_report_safe(r.clone()) })
            .map(|b| { b as i32 })
            .sum::<i32>()
    );
    println!(
        ...
        reports
            .iter()
            .map(|r: &Report| { problem_dampener(r.clone()) })
            .map(|b| { b as i32 })
            .sum::<i32>()
    )
}
